...
namespace AspNetCore.Identity.Dapper
{
    internal class RolesTable
    {
        ...
        public async Task<IdentityResult> CreateAsync(ApplicationRole role, CancellationToken cancellationToken) {
            const string command = "INSERT INTO [Identity].Roles " +
        ...
        public async Task<IdentityResult> UpdateAsync(ApplicationRole role) {
            const string command = "UPDATE [Identity].Roles " +
            ...
            using (var sqlConnection = await _databaseConnectionFactory.CreateConnectionAsync()) {
                using (var transaction = sqlConnection.BeginTransaction()) {
                    ...
                    if (role.Claims.Count() > 0) {
                        ...
                        const string insertClaimsCommand = "INSERT INTO [Identity].RoleClaims (RoleId, ClaimType, ClaimValue) " +
